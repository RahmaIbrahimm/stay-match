import 'package:flutter/material.dart'; // Changed from cupertino to material globally
...
import 'package:stay_match/Features/questions/presentation/views/questions_finish_pic.dart'; // 🌟 Ensure this points to your QuestionsFinishView
...
import '../../Features/reviews/presentation/views/show_reviews_view.dart';
...
class AppRouting {
  ...
  static const roomDetailsViewPath = 'room-details/:roomId';
  ...
  // =========== Reviews ============
  static const showReviewsPath = '/apartment-reviews/:propertyId';
  static const showReviewsName = 'apartmentReviews';
  ...
  static const secondarySplashPath = '/';
  ...
  static final router = GoRouter(
    ...
    initialLocation: secondarySplashPath, // Boots to splash to process token check sequence safely
    debugLogDiagnostics: true,
    routes: [
      // Initial Loader Screen
      GoRoute(
        path: secondarySplashPath,
        name: secondarySplashName,
        builder: (context, state) => const SecondarySplashView(),
      ),
      // Onboarding Intro Welcome Layout Page
      GoRoute(
        name: onboardingName,
        path: onboardingPath,
          ...
          GoRoute(
              ...
              routes: [
                GoRoute(
                  ...
                  builder: (context, state) {
                    final roomId = int.tryParse(state.pathParameters['roomId'] ?? '') ?? -1;
                    final propertyId = int.tryParse(state.pathParameters['propertyId'] ?? '') ?? -1;
      ...
      //       isRoom: extra['isRoom'] as bool? ?? false,
      //     );
      //   },
      // ),
      GoRoute(
        ...
        builder: (context, state) {
          ...
          // 🌟 Safe numerical conversion layer
          final rentValue = extra['monthlyRent'];
          final securityDepositValue = extra['securityDeposit'];
          final double parsedRent = rentValue is num ? rentValue.toDouble() : -1.0;
          final double parsedSecurityDeposit = securityDepositValue is num ? securityDepositValue.toDouble() : -1.0;
          ...
          return RequestBookingView(
            ...
            startDate: extra['startDate'] as String? ?? '',
            duration: extra['duration'] as int? ?? -1,
            monthlyRent: parsedRent, // 🌟 Safe value assignment
            city: extra['city'] as String? ?? '',
      ...
      // ---------------- REVIEWS SYSTEM ----------------
      GoRoute(
        path: showReviewsPath,
        name: showReviewsName,
        builder: (context, state) {
          // 1. Parse the path parameter (guaranteed to be there by the route setup)
          final propertyId = int.tryParse(state.pathParameters['propertyId'] ?? '-1') ?? -1;

          // 2. Grab 'isRoom' from queryParameters instead of pathParameters
          final String? isRoomRaw = state.uri.queryParameters['isRoom'];

          // 3. If it's missing, default to false. Otherwise, check if it's 'true'.
          final bool isRoom = isRoomRaw != null ? isRoomRaw.toLowerCase() == 'true' : false;

          return ShowReviewsView(
            propertyId: propertyId,
            isRoom: isRoom,
          );
        },      ),
      ...
      GoRoute(
        ...
        builder: (context, state) => const QuestionsStartView(), // 🌟 Fixed: Displays the introduction screen now
      ),
      GoRoute(
        path: questionsPath,
        name: questionsName,
        builder: (context, state) => BlocProvider(
          create: (context) => QuestionsCubit(questionsRepo: getIt.get<QuestionsRepoImpl>()),
          child: const QuestionsView(),
        ),
      ),
      GoRoute(
        name: questionsFinishName,
        path: questionsFinishPath,
        builder: (context, state) => const QuestionsFinishView(), // 🌟 Fixed: Connected questionnaire completion layout view
      ),
