...
class SavedPropertyCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        var route = item.itemType == "property"
            ? AppRouting.apartmentDetailsViewName
            : AppRouting.roomDetailsViewName;
        if (context.mounted) {
          if (item.itemType == "property") {
            context.pushNamed(route, pathParameters: {"id": item.propertyId.toString()});
          }
         else{
            context.pushNamed(route, pathParameters: {"propertyId": item.propertyId.toString(),"roomId":item.roomId.toString()});
          }
        }
      },
      child: Container(
