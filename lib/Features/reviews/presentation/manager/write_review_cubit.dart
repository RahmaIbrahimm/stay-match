  ...
  Future<void> submitReview({required WriteReviewRequest request}) async {
    ...
    result.fold(
        ...
        if (response.isSuccess == true) {
          emit(WriteReviewSuccess(message: response.message ?? 'Review submitted successfully'));
        } else {
          emit(WriteReviewFailure(errMessage: response.message ?? 'Failed to submit review'));
        }
      },
    );
  }
