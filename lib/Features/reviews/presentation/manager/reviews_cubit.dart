...
class ReviewsCubit extends Cubit<ReviewsState> {
  ...
  // ─── HOST REPLY ──────────────────────────────────────────────
  Future<void> hostReply({
    required int reviewId,
    required String response,
  }) async {
    emit(HostReplyLoading(reviewId: reviewId));

    final result = await reviewsRepo.hostReplyToReview(
      reviewId: reviewId,
      response: response,
    );
    ...
    result.fold(
          (failure) {
        emit(HostReplyFailure(reviewId: reviewId, errMessage: failure.errMessage));
      },
          (resp) {
        if (resp.success != true) {
          final msg = resp.error ?? resp.message ?? 'Failed to send reply';
          emit(HostReplyFailure(reviewId: reviewId, errMessage: msg));
          return;
        }
        emit(HostReplySuccess(reviewId: reviewId, response: response));
      },
    );
  }
}