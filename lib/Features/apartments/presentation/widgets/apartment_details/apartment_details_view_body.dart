...
//           'propertyImg': (details!.propertyImages?.isNotEmpty ?? false)
//               ? details.propertyImages![0].imageUrl
//               : null,          // 'hostImg': details.host?[0],
//         },
//       );
//     }
//   }
...
class _ApartmentDetailsViewBodyState extends State<ApartmentDetailsViewBody> {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<ApartmentDetailsCubit, ApartmentDetailsState>(
      ...
      builder: (context, state) {
        ...
        if (state is GetApartmentDetailsFailure) {
          return FailureState(errMessage: state.errMessage, id: widget.id);
        }
  ...
  Widget _buildSuccess(GetApartmentDetailsSuccess state) {
    final details = state.response.data;
    if (details == null) {
      return Center(
        child: Text(
    ...
    final bool initialSavedStatus = details.isSaved ?? false;

    if (details.allowedTenants?.allowsStudents ?? false) {
      allowedStudentTenantGender =
          details.allowedTenants?.studentGender?.trim().toLowerCase() ?? 'any';
    }

    final availableDate = details.availableFrom != null
        ? DateTime.tryParse(details.availableFrom!) ?? DateTime.now()
        : DateTime.now();

    final numBathrooms =
        (details.numberOfEnSuiteBathrooms ?? 0).toInt() +
            (details.numberOfGuestBathrooms ?? 0).toInt();
    final numBeds = details.numberOfBedrooms ?? 0;
    final amenities = Apartmentdetailshelper.getAmenitiesWithIcons(details.amenities);
    final numOfImages = details.propertyImages?.length ?? 0;
    final isMyProperty = details.isMyProperty ?? false;
              ...
              child: Stack(
                ...
                children: [
                  PageView.builder(
                    ...
                    itemCount: numOfImages,
                    onPageChanged: (value) => currentPic.value = value + 1,
                    itemBuilder: (context, index) {
                      final imageUrl = details.propertyImages?[index].imageUrl ?? '';
                      return CachedNetworkImage(
                        imageUrl: imageUrl,
                        fit: BoxFit.cover,
                        placeholder: (context, url) => Container(
                  ...
                  ValueListenableBuilder<int>(
                    ...
                    builder: (context, value, _) {
                      return Container(
                        margin: EdgeInsets.symmetric(vertical: 20.r, horizontal: 8.r),
                          ...
                          children: List.generate(numOfImages, (index) {
                            ...
                            return AnimatedContainer(
                              ...
                              decoration: BoxDecoration(
                                ...
                                color: isActive ? AppColors.containerColor : AppColors.grey,
                              ),
                            );
                          }),
                        );
                  ...
                  child: CustomElevatedButton(
                    ...
                    onPressed: () {
                      if (context.mounted) {
                        context.pushNamed(
                          AppRouting.showReviewsName,
                          pathParameters: {'propertyId': details.id.toString()},
                        );
                      }
                    },
                    backgroundColor: AppColors.bgGrey,
              ...
              children: [
                Wrap(
                  ...
                  children: [
                    ...
                    Container(
                      padding: EdgeInsets.symmetric(horizontal: 32.r, vertical: 4.r),
                      decoration: BoxDecoration(
                        color: AppColors.primary.withValues(alpha: 0.1),
                        borderRadius: BorderRadius.circular(15.r),
                      ),
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [_deposit(details), _minimumStay(details)],
                ),
                if (!isMyProperty) ...[
                  ...
                  DurationSelector(
                    ...
                    selectedDate: moveInDate,
                    selectedMonths: duration,
                    onDateChanged: (date) => setState(() => moveInDate = date),
                    onDurationChanged: (months) => setState(() => duration = months),
                  ),
                ],
              ],
            ),
  ...
  void _onBookNowPressed({required PropertyDetailsData? details}) {
    ...
    if (mounted) {
      final firstImage = (details?.propertyImages?.isNotEmpty ?? false)
          ? details!.propertyImages![0].imageUrl
          : null;
