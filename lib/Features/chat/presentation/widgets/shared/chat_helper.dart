...
class ChatHelper {
  ...
  static Widget buildPlaceholder(String? name, {int? fontSize}) {
    // 1. Safety check for null or completely empty name
    if (name == null || name.trim().isEmpty) {
      return _buildAvatarContainer("?", const Color(0xFF1B4F72), fontSize);
    }

    // 2. Filter out any accidental empty strings caused by double spaces
    List<String> nameParts = name.trim().split(' ')
    ...
    // 3. Now we are 100% sure elements aren't empty strings
    if (nameParts.length >= 2) {
      initials = nameParts[0][0] + nameParts[1][0];
    ...
    return _buildAvatarContainer(initials.toUpperCase(), _getAvatarColor(name), fontSize);
  }

  // Helper UI extractor to keep the code clean and scannable
  static Widget _buildAvatarContainer(String text, Color backgroundColor, int? fontSize) {
    return Container(
      width: 50.r,
      height: 50.r,
  ...
  static Map<String, List<Messages>> groupMessagesByDate(List<Messages> messages) {
    Map<String, List<Messages>> groups = {};

    for (var message in messages) {
      if (message.sentAt == null) continue; // Safety check
      final localDate = parseServerDate(message.sentAt!);
      final dateKey = getGroupDate(localDate);
